...
insect.tree = read.tree("insect_phylogeny_2017_collapsed_rerooted_calibrated.tre")

# Load the phenotype dataset (CSV with pre-wrangled data)
pheno.df = read.csv("pheno_processed_df_20250509.csv")
...
# For fig b of fig 3 panel
figB_df <- fits_df %>%
  dplyr::filter(model == "Mfull") %>%
  dplyr::select(map, q01_Acu, q01_HDother)


# Summary stats for publication (median + 2.5–97.5% CI across maps)
figB_summ <- figB_long %>%
...
setwd(res)
write.csv(figB_df,   "FigB_rates_per_simmap_Mfull_2017.csv", row.names = FALSE)
write.csv(figB_summ, "FigB_rates_summary_Mfull_2017.csv",    row.names = FALSE)
...
p <- p +
  ...
  scale_color_manual(
    ...
    labels = c("Aculeate haplodiploids", "Non-aculeate haplodiploids", "Diplodiploid")
  )
...
svg("CTMC_phylogeny_2017.svg", width = 10, height = 6)
print(p)
dev.off()
...
print(pB)
# Save
svg("CTMC_eus_rate_2017.svg", width = 6.5, height = 4.5)
print(pB)
dev.off()
...
# Optional save
svg("CTMC_model_comparison_2017.svg", width = 6.5, height = 4.5)
print(pC)
dev.off()
