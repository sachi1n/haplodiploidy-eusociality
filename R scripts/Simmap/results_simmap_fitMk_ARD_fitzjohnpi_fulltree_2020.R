...
simmap.trees = readRDS("hap_and_eus_simmap_fitMk_ARD_fitzjohnpi_fulltree_2020.RDS")
#-------------------------------------------------------------------------------
...
write.csv(clades.df, file = "clades_df_simmap_fulltree_2020.csv", row.names = FALSE)
clades.df = read.csv("clades_df_simmap_fulltree_2020.csv")
...
setwd(fig)
ggsave(
  filename = "eusocial_origins_clades_hapall_eusall_2020.svg",
  plot     = p1,
  device   = svglite,
  path     = fig,
  width    = 8,
