...
# NOTE: Haplodiploidy is used in a strict sense (HD.arrhenotoky). Eusociality is also used in the strict sense (Eusocial.strict)
...
simmap.trees = readRDS("hap_and_eus_simmap_fitMk_ARD_fitzjohnpi_haparrhenotoky_euskoos_halictidae_pruned.RDS")
simmap.trees=drop.tip(simmap.trees,"Paraponera_clavata")

saveRDS(simmap.trees, file = "hap_and_eus_simmap_fitMk_ARD_fitzjohnpi_haparrhenotoky_euskoos_halictidae_pruned_paraponera_removed.RDS")
...
# Add more clades to be conistent across all the definitions
new_clades <- data.frame(
  clade = rep(c("aphids", "platypodines", "halictids", "thrips"), each = 100),
  eusocial = rep(0, 4 * 100),
  ploidy = rep(c("DD", "DD", "HD", "HD"), each = 100)
)

# Combine the new clades with the existing clades_df
...
p1=ggplot(clades.df, aes(x = eusocial, y = clade, height = after_stat(density),
  ...
  geom_density_ridges(stat = "binline", binwidth = 1, bins=bins,scale = 0.7, binwidth = 0.5,
                      draw_baseline =TRUE, color = "grey70")+
  #  theme_ridges()+
...
p1
setwd(fig)
ggsave(filename = "eusocial_origins_clades_haparrh_euskoos_halpruned.svg", plot = p1, device = svglite, path = fig, width = 8, height = 6)
